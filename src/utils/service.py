...
def get_product_by_one_field(insert_dto_model, repository_func, status_code: int = 201, **kwargs):
    """
    ...
    try:
        ...
        return result
    searching_product_orm: "ProductOrm" = repository_func(
...
def update_product_by_one_field(insert_dto_model, repository_find_func, status_code: int = 200, **kwargs):
    """ """
    ### провалидировать kwargs через две модели ДТО: name+price, name
    try:
        product_searching_dto = insert_dto_model(**kwargs)  ### ProductUpdateByNameInsertDTO, ProductUpdateByIdInsertDTO
    ...
    updated_product_orm: "ProductOrm" = ProductRepository.update_last_version_of_existing_product_by_id_repository(
